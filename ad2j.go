...
func main() {
	...
	if absPath, err := filepath.Abs(exefile); err == nil {
		var sep string
		if runtime.GOOS == "windows" {
			sep = "\\"
		} else {
		...
		countryfile := dir[:strings.LastIndexAny(dir, sep)] + sep + "src" + sep + "ad2j" + sep + "country.json"
		bytes, err := ioutil.ReadFile(countryfile)
		if err != nil {
			panic(err)
		}

		if err := json.Unmarshal([]byte(bytes), &countrymap); err != nil {
			panic(err)
		}
	} else {
		panic(err)
